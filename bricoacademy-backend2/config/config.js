...
const config = {
	...
	db: {
		...
		name: process.env.DB_NAME || "bricoacademy",
...
logMensaje("DBNAME config:", config.db.name);
logMensaje("DBHOST config:", config.db.host);
logMensaje("DBUSER config:", config.db.user);
logMensaje("DBPORT config:", config.db.port);
